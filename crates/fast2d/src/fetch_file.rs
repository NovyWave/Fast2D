...
impl std::fmt::Display for FetchFileError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            FetchFileError::FetchFailed { url, error } => {
                write!(f, "Failed to fetch '{}': {}", url, error)
            }
            FetchFileError::ResponseCastFailed { url, error } => {
                write!(f, "Failed to cast response for '{}': {}", url, error)
            }
            FetchFileError::ArrayBufferFailed { url, error } => {
                write!(f, "Failed to get ArrayBuffer for '{}': {}", url, error)
            }
            FetchFileError::ArrayBufferPromiseFailed { url, error } => write!(
                f,
                "Failed to resolve ArrayBuffer promise for '{}': {}",
                url, error
            ),
            FetchFileError::BufferCastFailed { url, error } => {
                write!(f, "Failed to cast buffer for '{}': {}", url, error)
            }
        }
    }
}
...
pub async fn fetch_file(url: &str) -> Result<Vec<u8>, FetchFileError> {
    ...
    let browser_window = window().ok_or(FetchFileError::NoWindow {
        url: url.to_string(),
    })?;

    // Start the fetch and await the response
    let fetch_promise = browser_window.fetch_with_str(url);
    let fetch_response_jsvalue =
        JsFuture::from(fetch_promise)
            .await
            .map_err(|error| FetchFileError::FetchFailed {
                url: url.to_string(),
                error: format!("{error:?}"),
            })?;
    let response: Response =
        fetch_response_jsvalue
            .dyn_into()
            .map_err(|error| FetchFileError::ResponseCastFailed {
                url: url.to_string(),
                error: format!("{error:?}"),
            })?;
    ...
    let array_buffer_promise =
        response
            .array_buffer()
            .map_err(|error| FetchFileError::ArrayBufferFailed {
                url: url.to_string(),
                error: format!("{error:?}"),
            })?;
    let array_buffer_jsvalue = JsFuture::from(array_buffer_promise)
        .await
        .map_err(|error| FetchFileError::ArrayBufferPromiseFailed {
            url: url.to_string(),
            error: format!("{error:?}"),
        })?;
    let array_buffer = array_buffer_jsvalue
        .dyn_into::<web_sys::js_sys::ArrayBuffer>()
        .map_err(|error| FetchFileError::BufferCastFailed {
            url: url.to_string(),
            error: format!("{error:?}"),
        })?;
